                ...
                <tbody>
                    <?php if (count($estudios) > 0): ?>
                        <?php foreach ($estudios as $e): ?>
                            <tr>
                                <td><?= isset($e['AnioFiscal']) ? (int)$e['AnioFiscal'] : '-' ?></td>
                                ...
                                <td><?= $fechaText ?></td>
                                <td class="text-end">
                                    <a href="index.php?controller=catalogo&action=deleteEstudio&eliminar=<?= $e['Id'] ?>&id=<?= $catalogo['Id'] ?><?= $selectedYear !== null ? '&year=' . (int)$selectedYear : '' ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar estudio?')">Eliminar</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
